...
| TD-004 | Storage | Milestone 22 | API run storage is no longer hardcoded to local `outputs/`; deployments can set `CRAWLER_RUN_STORAGE_PATH` and `CRAWLER_STORAGE_BACKEND=shared_filesystem` while local JSONL mode remains the default. Managed PostgreSQL/object storage implementation is deferred to Milestone 27. | `python -m pytest tests/test_api.py` |
